...
namespace Roomify.Services.AutomaticMigrations
{
    public class SetupDevelopmentEnvironmentService
    {
        ...
        {
            var exist = await _userManager.FindByNameAsync("administrator@mail.com");
            if (exist != null)
            {
                return exist;
            }
            ...
            await _userStore.SetUserNameAsync(user, "administrator@mail.com", cancellationToken);
            await _userStore.SetEmailAsync(user, "administrator@mail.com", cancellationToken);
